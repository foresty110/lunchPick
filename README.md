...
3 Layer Architecture와 CRUD 구조를 바탕으로, JWT를 활용한 인증/인가 개념을 적용하고, 점심 라운드, 메뉴, 투표에 대한 비즈니스 로직을 구현하는 것에 중점을 둡니다.  

## 학습 목표
본 프로젝트를 통해 다음과 같은 핵심 목표를 달성하는 것을 목표로 합니다.

**1. 3 Layer 아키텍처 및 CRUD 구현**
Controller, Service, Repository 계층을 명확히 분리하고, Spring Data JPA를 활용하여 점심 라운드, 메뉴, 투표에 대한 CRUD 기능을 구현합니다.

**2. JWT 기반 인증/인가 시스템 적용**
JWT Access Token과 Refresh Token을 활용한 토큰 기반 인증 흐름을 이해하고 실제 코드에 적용합니다.
JWT의 sub (사용자 ID), role (권한) 클레임을 활용하여 데이터 접근 제어 및 인가 처리를 구현합니다.
Refresh Token 재발급 로직을 통해 토큰 만료 시 사용자 경험을 개선하는 흐름을 적용합니다.

**3. 비즈니스 로직 처리 및 안정성 확보**
ID당 투표 중복 제한과 같은 핵심 비즈니스 로직을 구현하며 데이터의 무결성을 유지합니다.
단위 테스트(Unit Test) 작성을 통해 각 기능의 동작을 검증하고 안정적인 API 개발을 경험합니다.
